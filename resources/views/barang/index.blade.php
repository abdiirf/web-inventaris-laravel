                ...
                <div class="card border-0 shadow rounded">
                    <div class="card-body">
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @forelse ($barang as $index => $barang1)
                                    <tr>
                                        <td>{{ $barang->firstItem() + $index }}</td>
                                        <td>{{ $barang1->nama_barang }}</td>
                                        <td class="text-center">
                                            <img src="{{ Storage::url('public/posts/').$barang1->image }}" class="rounded" style="width: 150px">
                                        </td>
                                        <td class="text-left">
                                            <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('barang.destroy', $barang1->id) }}" method="POST">
                                                <a href="{{ route('barang.edit', $barang1->id) }}" class="btn btn-sm btn-primary">EDIT</a>
                                                @csrf
                                                @method('DELETE')
